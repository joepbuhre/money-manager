...
// 		var err error
// 		user, err = database.GetUser(database.UserToken{Token: key}, self.config.DB)
// 		if err != nil {
// 			log.Println(err)
// 			c.AbortWithStatusJSON(http.StatusBadGateway, gin.H{"error": "Something went wrong"})
...
// 		c.AbortWithStatusJSON(http.StatusUnauthorized, self.config.ErrorMessages[1]) //"No Key Or Secret"
// 		return
// 	}
...
// 			c.AbortWithStatusJSON(http.StatusUnauthorized, self.config.ErrorMessages[2]) //"Unauthorized Access"
// 			return
...
// 			}
// 			c.AbortWithStatusJSON(http.StatusUnauthorized, self.config.ErrorMessages[99]) //"Software Developers have not setup authentication correctly"
// 			return
// 		}
// 		defer stmt.Close()
...
// func GetAuthMiddleware(db *sql.DB) func(c *gin.Context) {
// 	// Initialize Restgate
